...
class GrafikController extends Controller
{
    public function dashboardGrafik()
    {
        ...
        $dataTerakhirIsr = DB::table('broadcastings')
        ...
        if ($dataTerakhirIsr !== null) {
            $count_city_isr = DB::table('broadcastings')
    ...
    public function grafik(Request $request)
    {
        ...
        if (empty($bulan) || empty($tahun)) {
            $dataTerakhir = DB::table('broadcastings')
                ->select(DB::raw('MONTH(mon_query) as month, YEAR(mon_query) as year'))
                ->orderBy('mon_query', 'desc')
                ->first();
        ...
        $count_city = DB::table('broadcastings')
            ->select('city', DB::raw('COUNT(city) as total'))
        ...
        // send year to select
        $tahuns = Broadcasting::selectRaw('YEAR(mon_query) as tahuns')
            ->distinct()
            ->pluck('tahuns');
